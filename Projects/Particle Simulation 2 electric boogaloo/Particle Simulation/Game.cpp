...
void Game::resetWithXParticles(int x) {
	...
	for (int y = 0; y < toRender.size(); y++) {
		delete toRender.at(y);
	}
	for (int y = 0; y < toUpdate.size(); y++) {
		delete toUpdate.at(y);
	}
	toRender.clear();
	toUpdate.clear();
	particleList.clear();
	...
	for (int y = 0; y < x; y++) {
		Particle* p = new Particle(rand() % screenWidth, rand() % screenHeight, rand() % 15 + 15, (rand() % 1) - 1);
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_KEYDOWN:
				if (event.key.repeat == 0) {
					...
					case SDLK_0: resetWithXParticles(0 * 1); break;
					case SDLK_1: resetWithXParticles(1 * 1); break;
					case SDLK_2: resetWithXParticles(2 * 1); break;
					case SDLK_3: resetWithXParticles(3 * 1); break;
					case SDLK_4: resetWithXParticles(4 * 1); break;
